...
function afterFirstToken(input) {
    const spaceIndex = input.indexOf(' ');
 ...
 */
export const BUILT_IN_TEAM_PROFILES = {
    'research-review': {
...
export function listConfiguredProfiles(profiles) {
    ...
    for (const rawKey of keys) {
        ...
        seen.add(name);
        listed.push({ name, config: profiles_[rawKey] });
    }
    return listed;
}
...
export function parseProfileInvocation(rawInput) {
    ...
    if (head === PROFILE_FLAG_DASH) {
        ...
        if (name !== '')
            return { profile: name, goal: afterFirstToken(rest) };
        return { goal: trimmed };
    }
...
export function resolveTeamProfile(profiles, profileName, maxMembers) {
    ...
    if (entry === undefined)
        throw new Error(`AgentTeams profile "${profileName}" not found`);
    const config = entry.config;
    if (!isRecord(config))
        throw new Error(`profile "${profileName}" config is not an object`);
    // ── members ──
    ...
    const members = config.members.map((raw) => {
        if (!isRecord(raw))
            throw new Error(`profile "${profileName}" member is not an object`);
        ...
        if (!nonemptyString(raw['name']))
            throw new Error(`profile "${profileName}" member missing non-empty name`);
        const name = raw['name'].trim();
        if (name.toLowerCase() === 'captain')
            throw new Error(`profile "${profileName}" member name "captain" is reserved`);
        if (seenMemberNames.has(name.toLowerCase()))
            throw new Error(`profile "${profileName}" duplicate member name "${name}"`);
    ...
    const taskPlanning = rawTasks !== undefined && rawTasks.length > 0 ? 'seed' : 'captain';
    const seedIdOrder = (rawTasks ?? []).map((t) => (isRecord(t) ? String(t['id'] ?? '') : ''));
    const tasks = (rawTasks ?? []).map((raw, index) => {
        if (!isRecord(raw))
            throw new Error(`profile "${profileName}" task ${index} is not an object`);
        for (const key of Object.keys(raw)) {
        ...
        if (!nonemptyString(raw['id']))
            throw new Error(`profile "${profileName}" task ${index} missing non-empty id`);
        if (!nonemptyString(raw['subject']))
            throw new Error(`profile "${profileName}" task ${index} missing non-empty subject`);
    ...
    for (const task of tasks) {
        for (const dep of task.dependencies) {
            if (!taskIds.has(dep))
                throw new Error(`profile "${profileName}" task "${task.id}" depends on unknown task "${dep}"`);
        }
    }
    // ── fallback ──
    let fallback;
