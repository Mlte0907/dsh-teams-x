...
import { DISCOVERY_INTERVAL_MS, POLL_INTERVAL_MS, TeamsXPanelBody, useTeamData } from "./panel-body.js";
import { setSheetOpen } from "./sheet-visibility.js";
import { onTeamsXPanelRequest } from "./open-request.js";
...
export function ActivityPanel({ sessionId, t, openMember }) {
    ...
    useEffect(() => (onTeamsXPanelRequest((target) => {
        if (target === sessionId) {
            setExpanded(true);
            return true;
        }
    ...
    if (sessionTeams.length === 0 && !hasArchived && error === undefined && !expanded)
        return null;
    ...
    const badge = (_jsxs("button", { type: 'button', ref: badgeRef, className: css.badgeFab, "data-expanded": expanded === true || undefined, "data-busy": runningTasks > 0 || undefined, onClick: () => { setExpanded((value) => !value); }, "aria-label": badgeLabel, "aria-expanded": expanded === true || undefined, title: badgeDetail, children: [_jsx(TeamsXLogo, { size: 16, decorative: true }), workingCount > 0 && _jsx("span", { className: css.badgeFabBusy, "aria-hidden": true })] }));
    if (!expanded)
        return badge;
    return (_jsxs(_Fragment, { children: [badge, createPortal(_jsx("div", { className: isNarrow ? css.panelSheet : css.panelWindow, ref: panelRef, style: isNarrow ? undefined : {
                    top: `${placement.top}px`,
                    left: `${placement.left}px`,
                }, role: 'region', "aria-label": translate('panel.aria'), children: _jsx(TeamsXPanelBody, { sessionId: sessionId, t: t, openMember: openMember, onClose: () => { setExpanded(false); } }) }), document.body)] }));
}
