...
function IconSvg({ definition, label, size = 16, className, decorative }) {
    ...
    return (_jsx("svg", { viewBox: definition.viewBox ?? '0 0 24 24', width: size, height: size, fill: fill ? 'currentColor' : 'none', stroke: fill ? 'none' : 'currentColor', strokeWidth: fill ? undefined : ICON_STROKE.width, strokeLinecap: fill ? undefined : ICON_STROKE.linecap, strokeLinejoin: fill ? undefined : ICON_STROKE.linejoin, className: className, role: decorative === true ? undefined : 'img', "aria-hidden": decorative === true || undefined, "aria-label": decorative === true ? undefined : label, children: _jsx("g", { dangerouslySetInnerHTML: { __html: definition.body } }) }));
}
/** Build one named icon component from the shared data table. */
function makeIcon(name) {
    const definition = ICONS[name];
    function Component(props) {
        return _jsx(IconSvg, { definition: definition, label: props.label ?? definition.label, ...props });
    }
    return Component;
}
...
function makeGlyphIcon(body, label) {
    const definition = { label, body };
    function Component(props) {
        return _jsx(IconSvg, { definition: definition, label: props.label ?? label, ...props });
    }
    return Component;
}
/** Rotate-clockwise arrow, for the panel refresh control. */
export const GlyphRefresh = makeGlyphIcon(`<polyline points='23 4 23 10 17 10'/><path d='M20.49 15a9 9 0 1 1-2.12-9.36L23 10'/>`, 'refresh');
...
export const GlyphProgress = makeGlyphIcon(`<path d='M21 11.5a8.38 8.38 0 0 1-8.5 8.5 8.5 8.5 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 8.5-8.5 8.38 8.38 0 0 1 8.5 8.5z'/>`, 'progress');
/** Two stroke bars, for the per-member pause control. */
export const GlyphPause = makeGlyphIcon(`<rect x='6.5' y='4.5' width='4' height='15' rx='1'/><rect x='13.5' y='4.5' width='4' height='15' rx='1'/>`, 'pause');
/** Clock face, for the instrument row's elapsed reading. */
export const GlyphClock = makeGlyphIcon(`<circle cx='12' cy='12' r='8.5'/><path d='M12 7.5V12l3 2'/>`, 'elapsed');
/** Encircled i, for the member-channel info popover trigger. */
export const GlyphInfo = makeGlyphIcon(`<circle cx='12' cy='12' r='8.5'/><path d='M12 11v5'/><path d='M12 7.6v.2'/>`, 'details');
/** Mailbox tray, for the inbox line header. */
export const GlyphInbox = makeGlyphIcon(`<path d='M4.5 4.5h15v15h-15z'/><path d='M4.5 13.5h4.4a3.1 3.1 0 0 0 6.2 0h4.4'/>`, 'inbox');
