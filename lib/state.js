 ...
 */
export async function writeTaskArtifact(stateRoot, teamId, task, output) {
    if (output.length <= ARTIFACT_SPILL_CHARS)
        return { output };
...
export function appendTaskProgress(task, text) {
 ...
 */
export async function readTeamOperations(stateRoot, teamId, limit = 50) {
    try {
 ...
 */
export async function appendTeamOperation(stateRoot, teamId, entry) {
    try {
 ...
 */
export function crossProcessLockEnabled() {
...
async function acquireFileLock(key) {
    ...
    for (;;) {
        try {
            const content = { pid: process.pid, createdAt: Date.now() };
 ...
 */
function computeIndexChecksum(captains, members) {
    return createHash('sha256').update(JSON.stringify({ captains, members })).digest('hex').slice(0, 16);
}
function indexMemberOf(state) {
    const index = emptyIndex();
    index.captains[state.captainSessionId] = state.id;
    for (const member of state.members) {
        if (member.id !== '' && member.status !== 'removed')
            index.members[member.id] = state.id;
    }
    return index;
}
async function writeIndex(stateRoot, index) {
...
async function readIndex(stateRoot) {
    try {
        ...
        if (typeof parsed !== 'object' || parsed === null)
            return undefined;
        const record = parsed;
        if (typeof record['captains'] !== 'object' || record['captains'] === null)
            return undefined;
        if (typeof record['members'] !== 'object' || record['members'] === null)
            return undefined;
        const captains = record['captains'];
 ...
 */
async function indexTeam(stateRoot, state) {
    await withTeamLock(`index:${stateRoot}`, async () => {
        ...
        for (const key of Object.keys(index.captains)) {
            if (index.captains[key] === state.id)
                delete index.captains[key];
        }
        for (const key of Object.keys(index.members)) {
            if (index.members[key] === state.id)
...
async function unindexTeam(stateRoot, teamId) {
    await withTeamLock(`index:${stateRoot}`, async () => {
        const index = (await readIndex(stateRoot)) ?? emptyIndex();
        for (const key of Object.keys(index.captains)) {
            if (index.captains[key] === teamId)
                delete index.captains[key];
        }
        for (const key of Object.keys(index.members)) {
            if (index.members[key] === teamId)
                delete index.members[key];
        }
        await writeIndex(stateRoot, index);
    });
...
function coerceTeamState(value, expectedId) {
    ...
    const tasks = value['tasks'].map((task) => {
        ...
        for (const listKey of ['inScope', 'outOfScope', 'acceptance', 'verify']) {
            ...
            if (Array.isArray(list)) {
                ...
                if (filtered.length === 0)
                    delete cleaned[listKey];
                else
                    cleaned[listKey] = filtered;
            }
        }
        ...
        cleaned['dependencies'] = Array.isArray(dependencies)
            ? dependencies.filter((item) => typeof item === 'string')
            : [];
        return cleaned;
    });
 ...
 * generic failure.
 */
export function describeTeamStateError(value, expectedId) {
    if (!isRecord(value))
        return 'team.json is not a JSON object';
    if (value['id'] !== expectedId)
        return `id ${JSON.stringify(value['id'])} does not match directory "${expectedId}"`;
    if (typeof value['name'] !== 'string' || value['name'].trim() === '')
        return 'name must be a non-empty string';
    if (!isOptionalString(value['description']))
        return 'description must be a string when present';
    if (typeof value['captainSessionId'] !== 'string' || value['captainSessionId'] === '')
        return 'captainSessionId must be a non-empty string';
    ...
    if (badMember >= 0)
        return `members[${badMember}] is not a valid member record (needs string id/name, finite joinedAt, status idle|working|removed)`;
    if (!Array.isArray(value['tasks']))
        return 'tasks must be an array';
    const tasks = value['tasks'];
    const badTask = tasks.findIndex((task) => !isTeamTask(task));
    if (badTask >= 0)
        return `tasks[${badTask}] is not a valid task record (needs id/subject/status/dependencies/createdAt/updatedAt)`;
    if (!Number.isSafeInteger(value['taskSeq']) || value['taskSeq'] < 0)
        return 'taskSeq must be a non-negative safe integer';
    ...
    if (value['approvedAt'] !== undefined && !isFiniteNumber(value['approvedAt']))
        return 'approvedAt must be a finite number';
    if (value['halted'] !== undefined && typeof value['halted'] !== 'boolean')
        return 'halted must be a boolean';
    if (value['haltedAt'] !== undefined && !isFiniteNumber(value['haltedAt']))
        return 'haltedAt must be a finite number';
    const staged = value['phase'] === 'staged';
    const memberKeys = new Set();
    const memberIds = new Set();
    for (const [index, member] of value['members'].entries()) {
        const key = sanitizeKey(member.name);
        if (!staged && member.id === '')
            return `members[${index}] has an empty id on a non-staged team`;
        if (key === CAPTAIN_KEY)
            return `members[${index}] name collides with the reserved captain key`;
        if (memberKeys.has(key))
            return `members[${index}] name "${member.name}" collides with an earlier member`;
        if (member.id !== '' && memberIds.has(member.id))
            return `members[${index}] id collides with an earlier member`;
        memberKeys.add(key);
        if (member.id !== '')
            memberIds.add(member.id);
    }
    ...
    for (const [index, task] of value['tasks'].entries()) {
        if (task.id === '')
            return `tasks[${index}] has an empty id`;
        if (taskIds.has(task.id))
            return `tasks[${index}] id "${task.id}" collides with an earlier task`;
        taskIds.add(task.id);
    }
 ...
 */
export async function stateRootDiagnostics(stateRoot) {
    let entries;
    try {
...
export async function reindexTeam(stateRoot, state) {
    await indexTeam(stateRoot, state);
}
 ...
 */
async function rebuildIndex(stateRoot) {
    const index = emptyIndex();
    let entries;
    try {
        entries = await readdir(stateRoot, { withFileTypes: true });
...
async function scanForParticipant(stateRoot, agentSessionId) {
    let entries;
    try {
        entries = await readdir(stateRoot, { withFileTypes: true });
    }
 ...
 */
export async function listAllTeams(stateRoot) {
    let entries;
    try {
        entries = await readdir(stateRoot, { withFileTypes: true });
    }
...
export async function findTeamByCaptain(stateRoot, captainSessionId) {
    ...
    if (team !== undefined && team.captainSessionId !== captainSessionId)
        return undefined;
    return team;
}
...
const retiredMembersCache = new Map();
 ...
 */
export function unsatisfiedDependencies(tasks, dependencies, byId) {
 ...
 */
export function taskVisualState(status, dependencies, tasks, byId) {
    if (status === 'completed')
        return 'completed';
    if (status === 'failed')
        return 'failed';
...
export function buildMemberByNameIndex(members) {
    const index = new Map();
...
export function findMemberByName(index, name) {
