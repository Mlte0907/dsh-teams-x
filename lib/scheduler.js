...
import { acknowledgeMailbox, appendMailbox, appendTeamOperation, beginTaskAttempt, cancelUnfinishedTask, CAPTAIN_KEY, claimMailboxDelivery, createMessage, findTeamByParticipant, invalidateTaskAttempt, listAllTeams, readTeam, readUnreadMailbox, releaseMailboxDelivery, unsatisfiedDependencies, withTeamLock, writeTeam, } from "./state.js";
import { steerCaptainReport } from "./members.js";
 ...
 */
export function verdictRequiresRepair(verdict) {
    return verdict === 'needs_revision' || verdict === 'reject';
}
/**
 * Check if a task has reached the repair round limit.
 */
export function hasReachedRoundLimit(task, maxRounds) {
 ...
 */
export function deriveRepairTask(failedTask, findings, taskSeq) {
    const now = Date.now();
 ...
 */
export function openRepairSiblingFor(tasks, sourceTaskId) {
 ...
 */
export function cancelSupersededRepairSiblings(tasks, sourceTaskId) {
    const cancelled = [];
    for (const task of tasks) {
        if (task.id === sourceTaskId || task.kind !== 'repair')
            continue;
        if (!task.dependencies.includes(sourceTaskId))
            continue;
 ...
 */
export function classifyStall(task, ownerLive, now) {
 ...
 */
export function isStrandedCaptainTask(task, captainRunning) {
...
export function installTeamScheduler(ctx, config) {
    ...
    const trackRoot = (workspace) => {
        seenWorkspaces.add(workspace);
        return stateRootOf(workspace, config);
    };
    const reconcileStrandedCaptainTasks = async (workspace, teamId) => {
        ...
        await withTeamLock(teamLockKey(stateRoot, teamId), async () => {
            ...
            for (const task of team.tasks) {
                if (!isStrandedCaptainTask(task, captainRunning))
                    continue;
     ...
     */
    const detectStalledMemberWork = async (workspace, teamId) => {
        const stateRoot = trackRoot(workspace);
        const now = Date.now();
        let requeued = false;
        const notifications = [];
        await withTeamLock(teamLockKey(stateRoot, teamId), async () => {
            ...
            for (const task of team.tasks) {
                ...
                if (stallNotified.has(notifyKey))
                    continue;
    ...
    const runtime = {
        ...
        async triggerRepairLoop(workspace, teamId, taskId) {
            ...
            if (repairConfig?.autoDerive === false)
                return;
            ...
            const prepared = await withTeamLock(teamLockKey(stateRoot, teamId), async () => {
                ...
                if (!verdictRequiresRepair(task.verdict))
                    return;
                ...
                if (hasReachedRoundLimit(task, maxRounds)) {
                    ...
                    const message = {
                        ...createMessage('teamsx', CAPTAIN_KEY, `Task ${taskId} ("${task.subject}") has failed through ${maxRounds} repair rounds; automatic repair stopped. Review the findings on the task and decide: reassign with a sharper contract, split the task, or drop it.`),
