...
import { drainChildren, readTokenUsage } from "./compat.js";
import { teamsXNoticeSource } from "./message-source.js";
import { appendTaskProgress, appendTeamOperation, writeTaskArtifact, acknowledgeMailbox, appendMailbox, archiveTeamDir, beginTaskAttempt, CAPTAIN_KEY, createMessage, createTeamDir, findTeamByParticipant, listTeamsForParticipant, buildMemberByNameIndex, cancelUnfinishedTask, findMemberByName, invalidateTaskAttempt, readTeam, recordRetiredMemberIds, releaseMailboxDelivery, reindexTeam, readUnreadMailbox, sanitizeKey, stateRootDiagnostics, unsatisfiedDependencies, withTeamLock, writeTeam, } from "./state.js";
import { appendTeamEvent, captainSessionOf, messageEventContent } from "./events.js";
import { evaluateQualityCompletion, isQualityKind, normalizeBlankOptionalTaskFields, taskKindOf, transitionError, validateCreateTask, } from "./quality.js";
import { cancelSupersededRepairSiblings, verdictRequiresRepair, } from "./scheduler.js";
...
import { parseProfileInvocation, resolveTeamProfile, } from "./profiles.js";
 ...
 */
export function parseStagedPlanMutations(input) {
    ...
    const nonEmpty = (value, label, index) => {
        if (typeof value !== 'string' || value.trim() === '') {
            throw new Error(`mutation ${index}: "${label}" must be a non-empty string`);
        }
        return value;
    };
    const optionalStringOrNull = (value) => (value === null ? null : typeof value === 'string' && value.trim() !== '' ? value : undefined);
    const dependencies = (value, index) => {
    ...
    return input.map((raw, index) => {
        ...
        switch (mutation['action']) {
            ...
            case 'remove_task':
                return { action: 'remove_task', taskId: nonEmpty(mutation['taskId'], 'taskId', index) };
            case 'remove_member':
                return { action: 'remove_member', memberName: nonEmpty(mutation['memberName'], 'memberName', index) };
            default:
...
function requireCaptain(exec) {
    if (!exec.agent) {
        throw new Error('teamsx tools require a calling agent (exec.agent was undefined)');
    }
    return exec.agent;
}
...
async function authorizationMissHint(stateRoot) {
...
/** Fresh state for a team that still exists; never falls back to stale data. */
...
function requireTask(team, taskId) {
    ...
    if (task === undefined) {
        throw new Error(`no task "${taskId}" in team "${team.name}" — use teamsx_status to list tasks`);
    }
    return task;
}
...
async function stopTeamMemberActivations(ctx, captain, members, signal) {
    ...
    try {
        await drainChildren(ctx, captain, memberIds);
        return;
    }
    catch (error) {
        // Do not claim the stop request succeeded when the runtime could not
        // release all selected child activations.
        ctx.logger.warn(`teams-x: failed to drain halted members: ${String(error)}`);
        throw error;
    }
}
export async function haltTeamWork(input) {
    const halted = await withTeamLock(teamLockKey(input.stateRoot, input.teamId), async () => {
        ...
        appendTeamEvent(input.ctx, captainSessionOf(input.ctx, fresh.captainSessionId, input.captain.session), 'teamsx/team-halted', {
            teamId: fresh.id,
            cancelledTasks,
        });
        return {
    ...
    // can start a fresh Captain turn after the first cancellation; close the
    // stop boundary again once every child activation has drained. This
    // double-cancel is defense-in-depth: missing it causes silent team resume.
    input.captain.cancel({ kind: 'user' }, { keepInbox: true });
    return {
...
export function stagedPlanDiscardContext(teamName) {
    return [
        `The user discarded the staged TeamsX plan "${teamName}" from the pre-run review surface.`,
        'That decision is final for this draft: it has been archived, no members were created, and no tasks may run.',
        'Do not call teamsx_create, teamsx_approve, or recreate a replacement team merely because the old team is no longer active.',
        'Wait for a later explicit user request. If the next user message is unrelated to TeamsX, answer it normally and do not start a team.',
    ].join('\n');
}
 ...
 */
export function registerTeamsXTools(ctx, config) {
    ...
    const approveStagedTeam = async (captain, teamId, signal) => {
        ...
        const approved = await withTeamLock(teamLockKey(stateRoot, teamId), async () => {
            const fresh = await requireFreshCaptainTeam(stateRoot, teamId, captain.id);
            requireStagedTeam(fresh);
            // A staged removal has no child session to retain in history. Drop
            // those placeholders before transitioning to the stricter running shape.
            ...
            try {
                ...
                for (const member of fresh.members) {
                    ...
                    const selection = await resolveMemberLlmSelection(ctx, captain, {
                        ...
                        fallback: member.fallback ?? config.fallback,
                    }, runSignal);
                ...
                for (const [member, selection] of selections) {
                    await spawnMember(ctx, memberRuntime(), memberSelections, selection, captain, fresh, member, config.stateDir, runSignal);
                    spawned.push(member);
                }
                ...
                await writeTeam(stateRoot, fresh);
                await reindexTeam(stateRoot, fresh);
                appendTeamEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, captain.session), 'teamsx/team-approved', {
                    teamId: fresh.id,
                    members: fresh.members.length,
        ...
        try {
            ...
            // recoverable by the next status/member lifecycle kick.
            ctx.logger.warn(`teams-x: post-approval kick failed for "${teamId}": ${String(error)}`);
        }
        return approved;
    };
    ...
    const discardStagedTeam = async (captain, teamId) => {
        ...
        const discarded = await withTeamLock(teamLockKey(stateRoot, teamId), async () => {
            const fresh = await requireFreshCaptainTeam(stateRoot, teamId, captain.id);
            requireStagedTeam(fresh);
            appendTeamEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, captain.session), 'teamsx/plan-discarded', {
                teamId: fresh.id,
            });
        ...
        // Preserve this control fact for the next genuine user turn, then abort
        // the still-running Captain turn. Without both operations a late model
        // step can observe the missing active team and incorrectly recreate it.
        try {
    ...
    ctx.tools.register(defineTool({
        ...
        parameters: {
            ...
            approval: {
                ...
                description: 'required stages the plan for explicit user review; automatic starts immediately. Defaults to automatic.',
            },
        },
        output: {
            schema: {
            ...
            render: (_args, value) => [{
                    ...
                    text: value.phase === 'staged'
                        ? `Team "${value.team_name}" plan created under ${value.state_dir}. It is staged: finish the roster and DAG, then wait for the user to approve it. Do not start or approve it yourself.`
        ...
        async execute(args, exec) {
            ...
            if (staged === false) {
                try {
                    await scheduler.kickTeam(workspace, created.id, captain);
                }
                catch (error) {
                    ctx.logger.warn(`teams-x: post-create kick failed for "${created.id}": ${String(error)}`);
                }
            }
            appendTeamEvent(ctx, captain.session, 'teamsx/team-created', {
                teamId: created.id,
                captainSessionId: captain.id,
                name: created.name,
    ...
    // ── teamsx_edit_plan ──
    ctx.tools.register(defineTool({
        name: 'teamsx_edit_plan',
        description: 'Atomically revise the current staged TeamsX plan without spawning members or scheduling tasks. Use this when the user continues chatting to change a plan that is waiting for approval. Submit dependent edits in order (update downstream dependencies or assignees, then remove tasks, then remove unused members).',
        parameters: {
            operations: {
                type: 'array',
                required: true,
                ...
                items: {
                    ...
                    properties: {
                        ...
                        provider: { type: 'string', description: 'Optional member provider; requires model.' },
                        model: { type: 'string', description: 'Optional member model.' },
    ...
    ctx.tools.register(defineTool({
        ...
        parameters: {
            ...
            reasoning_effort: { type: 'string', description: 'Optional reasoning effort override, or "default" for the target model\'s default.' },
        ...
        output: {
            ...
            render: (args, value) => [{
                    ...
                    text: value.phase === 'staged'
                        ...
                        : `Member "${value.member_name}" added (subagent id ${value.member_id}, ${value.provider}/${value.model}, status ${value.status}).`,
                }],
        },
        async execute(args, exec) {
            const captain = requireCaptain(exec);
            ...
            const created = await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                ...
                if (fresh.phase !== 'staged') {
                    await spawnMember(ctx, memberRuntime(), memberSelections, selection, captain, fresh, member, config.stateDir, exec.signal);
                }
                ...
                try {
                    ...
                    // record never saw it. Retire the orphan and surface the failure.
                    if (member.id !== '') {
                ...
                appendTeamEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, captain.session), 'teamsx/member-added', {
                    teamId: fresh.id,
                    memberId: member.id,
                    name: member.name,
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const revoked = await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                ...
                await writeTeam(stateRoot, fresh);
                await reindexTeam(stateRoot, fresh);
                appendTeamEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, captain.session), 'teamsx/member-removed', {
                    teamId: fresh.id,
                    memberId: member.id,
                });
    ...
    // ── teamsx_create_task ──
    ctx.tools.register(defineTool({
        name: 'teamsx_create_task',
        description: 'Create a task in your team\'s task list. Every call must include a non-empty subject. Tasks can depend on other tasks: a task is only claimable once every dependency is completed. Optionally assign it to a member, who still claims it before working.',
        parameters: {
            ...
            kind: {
                ...
                description: 'Task kind. Defaults to work. Quality kinds require a contract (objective, acceptance, and for write kinds inScope+verify).',
            },
        ...
        async execute(args, exec) {
            ...
            // them. Normalize blank optional fields to omitted before validation.
            const input = normalizeBlankOptionalTaskFields(args);
            const created = await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                const fresh = await requireFreshCaptainTeam(stateRoot, team.id, captain.id);
                if (fresh.halted === true) {
                    throw new Error('team is halted; call teamsx_resume with a reason first');
                }
                const gate = validateCreateTask(fresh.tasks, {
                ...
                await writeTeam(stateRoot, fresh);
                appendTeamEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, captain.session), 'teamsx/task-created', {
                    teamId: fresh.id,
                    taskId: task.id,
    ...
    ctx.tools.register(defineTool({
        name: 'teamsx_reassign_task',
        description: 'Atomically retry, reassign, or let the captain take over one ready unfinished/failed task. The old attempt is revoked before its member is interrupted, so late updates cannot overwrite the new owner. Use assignee="captain" only when you will finish that task in this turn.',
        parameters: {
        ...
        output: {
            ...
            render: (_args, value) => [{
                    type: 'text',
                    text: `Task ${value.task_id} reassigned ${value.previous_assignee || 'unassigned'} → ${value.assignee} (attempt ${value.attempt}, status ${value.status}).`,
                }],
        },
        async execute(args, exec) {
            const captain = requireCaptain(exec);
            ...
            const task = current === undefined ? undefined : current.tasks.find((candidate) => candidate.id === args.task_id);
            if (task === undefined)
    ...
    ctx.tools.register(defineTool({
        ...
        parameters: {
            ...
            acceptanceResults: {
                ...
                description: 'Acceptance evidence in contract order.',
            },
            commandsRun: {
                type: 'array',
                items: {
                ...
                description: 'Verification evidence in contract order.',
            },
        },
        output: {
            schema: {
            ...
            const updated = await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                ...
                if (TERMINAL_TASK_STATUSES.includes(task.status)) {
                    ...
                    if (!sameStatus || !sameOutput) {
                        throw new Error(`terminal task ${task.id} is immutable; use teamsx_reassign_task to retry failed/cancelled work`);
                    }
                    return {
                        task_id: task.id,
                        taskKind: taskKindOf(task),
                ...
                // Blank optional list entries must not be persisted: they would brick
                // the durable state on reload.
                const findings = parseFindings(input.findings);
                const acceptanceResults = parseAcceptanceResults(input.acceptanceResults);
                const commandsRun = parseCommandResults(input.commandsRun);
                ...
                if (args.output !== undefined) {
                    ...
                    if (spilled.artifact !== undefined)
                        task.artifact = spilled.artifact;
                }
                if (contractRevisionRequested) {
                    const previous = {
                    ...
                    if (args.inScope !== undefined)
                        task.inScope = args.inScope;
                    if (args.outOfScope !== undefined)
                        task.outOfScope = args.outOfScope;
                    if (args.acceptance !== undefined)
                        task.acceptance = args.acceptance;
                    if (args.verify !== undefined)
                        task.verify = args.verify;
                }
                if (TERMINAL_TASK_STATUSES.includes(task.status)) {
                    ...
                    if (usage !== undefined) {
                        ...
                        if (member !== undefined)
                            member.usage = { inputTokens: usage.inputTokens, outputTokens: usage.outputTokens };
                    }
                }
                ...
                if (changedPaths !== undefined)
                    task.changedPaths = changedPaths;
    ...
    // ── teamsx_send_message ──
    ctx.tools.register(defineTool({
        name: 'teamsx_send_message',
        description: 'Send a message to the captain or to a teammate. Messages go straight into the recipient\'s mailbox; when the recipient is online the plugin also schedules live delivery (member recipients get the message as their next turn; a running captain sees it at the nearest model step). Teammates talk to each other directly, no relay involved.',
        parameters: {
            ...
            from: { type: 'string', description: 'Sender (defaults to the caller; impersonation is rejected).' },
        },
        output: {
            schema: {
                type: 'object',
        ...
        async execute(args, exec) {
            ...
            const prepared = await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                ...
                if (args.from !== undefined && args.from !== from) {
                    throw new Error(`teamsx_send_message: "from" must be your own identity ("${from}"), not "${args.from}"`);
                }
                if (to === CAPTAIN_KEY) {
                    ...
                    appendTeamEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, caller.session), 'teamsx/message-sent', {
                        teamId: fresh.id,
                ...
                if (fresh.halted === true) {
                    throw new Error(`team "${fresh.name}" is halted; call teamsx_resume before waking a member`);
                }
            ...
            if (prepared.kind === 'captain') {
                ...
                await withTeamLock(teamLockKey(stateRoot, prepared.fresh.id), () => delivered === 'live'
            ...
            if (captain !== undefined && prepared.recipient.id !== '') {
                ...
                const text = `TeamsX state policy: inspect ${config.stateDir}/${prepared.fresh.id}/ read-only; never edit team.json or inbox files directly. Use teamsx_* tools for team state.\n\n${senderText}`;
        ...
        async execute(_args, exec) {
            ...
            for (const member of visibleMembers) {
                ...
                if (messages.length > 0) {
                    ...
                    memberInboxIds.set(member.name, messages.map((m) => m.id));
                }
            }
            const result = {
                team_id: team.id,
            ...
            const acknowledged = identity.kind === 'captain'
                ...
                : memberInboxIds.get(identity.name) ?? [];
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            const result = await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                const fresh = await requireFreshCaptainTeam(stateRoot, team.id, captain.id);
                if (args.reason.trim() === '')
                    throw new Error('a non-empty resume reason is required');
                if (fresh.halted !== true)
                    return { status: 'already_running', teamId: fresh.id };
                fresh.halted = false;
                fresh.haltedAt = undefined;
                await writeTeam(stateRoot, fresh);
                appendTeamEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, captain.session), 'teamsx/team-resumed', {
                    teamId: fresh.id,
                    reason: args.reason,
                });
                return { status: 'resumed', teamId: fresh.id };
            });
            if (result.status === 'resumed')
                await scheduler.kickTeam(workspace, team.id, captain);
            return { status: result.status, team_id: result.teamId, reason: args.reason };
        },
    }));
    // ── teamsx_delete ──
    ctx.tools.register(defineTool({
        name: 'teamsx_delete',
        description: 'End your team: interrupts all members (best effort) and archives the team\'s state directory (team file, tasks, mailboxes) for later review. Use when the team\'s work is done or abandoned.',
        parameters: {},
        output: {
            schema: {
                type: 'object',
        ...
        async execute(_args, exec) {
            ...
            const members = await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                const fresh = await requireFreshCaptainTeam(stateRoot, team.id, captain.id);
                // Include previously removed members so deleting a team also retires
                // durable catalog entries left behind by remove_member.
            ...
            for (const result of quiescence) {
                if (result.status === 'rejected') {
                    ctx.logger.warn(`teams-x: member did not quiesce cleanly before team archive: ${String(result.reason)}`);
                }
            }
            await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                const fresh = await requireFreshCaptainTeam(stateRoot, team.id, captain.id);
                appendTeamEvent(ctx, captainSessionOf(ctx, fresh.captainSessionId, captain.session), 'teamsx/team-deleted', {
                    teamId: fresh.id,
                });
                // Archive, not delete: tasks and mailboxes stay on disk for review.
                await archiveTeamDir(stateRoot, fresh.id);
            });
...
function renderStatus(value) {
    ...
    const lines = [];
    lines.push(`Team: ${String(value.team_name)} (phase ${String(value.phase)}${value.halted === true ? ', HALTED' : ''})`);
    for (const member of members ?? []) {
        lines.push(`  member ${member.name} (${member.role || 'member'}): ${member.status}/${member.activity}`);
    }
    for (const task of tasks ?? []) {
        lines.push(`  ${task.id} [${task.status}] ${task.subject} → ${task.assignee || 'shared'}${task.dependencies.length > 0 ? ` (after ${task.dependencies.join(', ')})` : ''}`);
    }
    return lines.join('\n');
}
function parseFindings(value) {
    ...
    return value.map((item, index) => {
        ...
        return {
            ...
            // Blank optional file must be omitted, not persisted.
