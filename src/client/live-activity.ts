...
import { peekSessions } from './client-runtime.ts'
 ...
 */
export function useLiveBeats(memberIds: readonly string[]): LiveBeats {
  const [beats, setBeats] = useState<LiveBeats>({})
  ...
  useEffect(() => {
    ...
    const publish = (id: string, running: boolean): void => {
      setBeats((prev) => {
        const current = prev[id]
        if (current !== undefined && current.running === running) return prev
    ...
    for (const id of ids) {
      ...
      try {
        const scope = sessions.scope(id as Parameters<typeof sessions.scope>[0])
...
export function beatActivity(
  ...
  beat: LiveBeat | undefined,
  now: number = Date.now(),
): 'working' | 'idle' | 'unknown' {
  if (beat === undefined || now - beat.seenAt > BEAT_TTL_MS) return activity
