...
export function memberInkIndex(name: string): number {
  let hash = 0x811c9dc5
 ...
 */
const SIGIL_RULES: readonly (readonly [RegExp, MemberSigilKey])[] = [
 ...
 */
export function memberSigil(name: string, role: string): MemberSigilKey | undefined {
  const identity = `${name} ${role}`.toLowerCase()
  for (const [pattern, sigil] of SIGIL_RULES) {
    if (pattern.test(identity)) return sigil
  }
  return undefined
}
