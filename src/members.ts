...
export function installMemberSelectionRuntime(
  ...
  const disposeSessionStart = ctx.on('agent/session-start', (payload) => {
    ...
    const disposeFallback = childCtx.on('agent/request-error', async (payload, next) => {
      ...
      const transition = selectFallbackRoute(
        selectionRef.current ?? { provider: selection.provider, model: selection.model },
        fallback,
      ...
      if (fallback !== undefined && transition.retry) {
        ...
        await updateFallbackState(stateRoot, teamId, memberName, fallback).catch((error: unknown) => {
...
async function recordMemberTurnFailure(
  ctx: Context,
  stateRoot: string,
  teamId: string,
  memberName: string,
  failure: { readonly code: string; readonly message: string },
  fallbackSession: Parameters<typeof captainSessionOf>[2],
...
export function memberWelcome(team: TeamState, memberName: string): string {
  const assigned = team.tasks.filter((task) => (
...
export async function spawnMember(
  ...
  if (provider === undefined) {
    throw new Error(
      `teams-x: no subagent provider "${config.provider}" is registered (available: ${ctx.subagents.list().join(', ') || 'none'}) — `
...
export function installRetiredMemberGuard(ctx: Context, stateDir: string): void {
  type DeliveryFn = (first: Agent, second: SessionId, content: unknown, options: unknown) => Promise<unknown>
  ...
  const patchable = ctx.subagents as unknown as Record<'sendMessage' | 'followup', unknown>
  const receiver = ctx.subagents as unknown as object
  ctx.effect(() => {
    const restores: Array<() => void> = []
    const isRetired = async (parent: Agent, childId: SessionId): Promise<boolean> => {
      const retired = await readRetiredMemberIds(join(parent.session.header.cwd ?? process.cwd(), stateDir))
      return retired.has(childId)
    }
    const guard = (methodName: 'sendMessage' | 'followup'): void => {
      const original = patchable[methodName] as DeliveryFn | undefined
      if (original === undefined) return
      const guarded: DeliveryFn = async (sender, targetId, content, options) => {
      ...
      restores.push(() => {
        if (patchable[methodName] === (guarded as unknown)) patchable[methodName] = original as unknown
      })
    }
    guard('sendMessage')
    guard('followup')
