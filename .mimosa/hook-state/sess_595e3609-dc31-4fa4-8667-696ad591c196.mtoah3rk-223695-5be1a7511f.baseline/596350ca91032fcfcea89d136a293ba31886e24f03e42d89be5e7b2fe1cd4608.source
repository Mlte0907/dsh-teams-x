...
function isDefaultExcluded(path: string): boolean {
  ...
  if (segments[0] === '.git' || segments[0] === '.teams-x') return true
...
export function normalizeBlankOptionalTaskFields<T extends Record<string, unknown>>(input: T): T {
  ...
  for (const listKey of ['inScope', 'outOfScope', 'acceptance', 'verify'] as const) {
    ...
    if (Array.isArray(value)) {
      ...
      if (cleaned.length === 0) delete next[listKey]
      else (next as Record<string, unknown>)[listKey] = cleaned
    }
  }
  return next
}
...
export function validateCreateTask(
...
): { ok: boolean; error?: string; kind: TaskKind } {
  ...
  if (WRITE_KINDS.includes(kind) && nonemptyStringList(input.inScope)) {
    ...
    for (const other of tasks) {
      ...
      if (other.dependencies.some((id) => byId.get(id) === undefined)) continue
      const overlap = (input.inScope ?? []).filter((a) => (other.inScope ?? []).some((b) => (
        pathMatchesScope(a, b) || pathMatchesScope(b, a) || a === b
      )))
      if (overlap.length > 0) {
        return {
          ok: false,
