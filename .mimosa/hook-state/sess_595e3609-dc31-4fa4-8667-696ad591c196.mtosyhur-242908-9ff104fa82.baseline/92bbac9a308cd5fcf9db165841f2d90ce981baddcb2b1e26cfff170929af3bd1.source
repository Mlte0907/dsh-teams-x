...
import { haltTeamWork, registerTeamsXTools, } from "./tools.js";
import { interruptMember } from "./members.js";
import { WEB_SERVER_KEYS, WORKSPACE_KEYS } from "./compat.js";
import { installKnownEventTypes } from "./events.js";
...
5. If the user explicitly asks to pause a running member, its open attempt remains parked after interruption; after answering the user, send that same member guidance with teamsx_send_message so it continues the same attempt. If work must change owner, restart from scratch, or be taken over, call teamsx_reassign_task first. Use assignee=captain only for one ready task that you will personally drive to a terminal status in this same turn; never start a second captain takeover while one is unfinished, and never end your turn with captain-owned work open.
