...
export function registerTeamsXTools(ctx: Context, config: ToolsConfig): TeamsXRuntime {
  ...
  const updateStagedPlanBatch: TeamsXRuntime['updateStagedPlanBatch'] = async (captain, teamId, mutations, signal) => {
  ...
  const updateStagedPlan: TeamsXRuntime['updateStagedPlan'] = async (captain, teamId, mutation, signal) => (
    updateStagedPlanBatch(captain, teamId, [mutation], signal)
  )

  const approveStagedTeam: TeamsXRuntime['approveStagedTeam'] = async (captain, teamId, signal) => {
    const workspace = workspaceOf(captain)
    const stateRoot = stateRootOf(workspace, config)
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const input = normalizeBlankOptionalTaskFields(args as Record<string, unknown>)
      const created = await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
        const fresh = await requireFreshCaptainTeam(stateRoot, team.id, captain.id)
        if (fresh.halted === true) {
        ...
        const gate = validateCreateTask(fresh.tasks, {
          ...
          kind: input.kind as import('./types.ts').TaskKind | undefined,
          round: input.round as number | undefined,
          objective: input.objective as string | undefined,
          inScope: input.inScope as string[] | undefined,
          outOfScope: input.outOfScope as string[] | undefined,
        ...
        const task: TeamTask = {
          ...
          updatedAt: Date.now(),
          kind,
          ...(input.round !== undefined ? { round: input.round as number } : {}),
          ...(input.objective !== undefined ? { objective: input.objective as string } : {}),
          ...(input.inScope !== undefined ? { inScope: input.inScope as string[] } : {}),
          ...(input.outOfScope !== undefined ? { outOfScope: input.outOfScope as string[] } : {}),
          ...(input.acceptance !== undefined ? { acceptance: input.acceptance as string[] } : {}),
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const updated = await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
        ...
        const changedPaths = input.changedPaths as string[] | undefined
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(_args, exec) {
      ...
      const memberInboxIds = new Map<string, string[]>()
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const result = await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
        const fresh = await requireFreshCaptainTeam(stateRoot, team.id, captain.id)
        if (args.reason.trim() === '') throw new Error('a non-empty resume reason is required')
        if (fresh.halted !== true) return { status: 'already_running', teamId: fresh.id }
        fresh.halted = false
...
function renderStatus(value: Record<string, unknown>): string {
  const tasks = value.tasks as Array<{ id: string; subject: string; status: string; assignee: string; dependencies: string[] }> | undefined
  const members = value.members as Array<{ name: string; status: string; activity: string; role: string }> | undefined
  const lines: string[] = []
